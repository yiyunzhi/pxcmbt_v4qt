...
class SearchMenuWidget(QtWidgets.QMenu):
    ...
    def _get_node_class_info_by_display_name(self, name):
        _res = None
        for k, v in self._nodeDict.items():
            for x in v:
    ...
    def _on_search_submitted(self):
        if not self._blockSubmit:
            ...
            _text = _action.text()
            _node_cls_info = self._get_node_class_info_by_display_name(_text)
            if _node_cls_info:
                self.sigSearchSubmitted.emit(_node_cls_info.class_type)
        self._close()

    def build_menu_tree(self):
        _node_names = sorted(self._nodeDict.keys())
        _menu_tree = OrderedDict()
        for group_name in _node_names:
            _new_menu = QtWidgets.QMenu(group_name)
            _new_menu.keyPressEvent = self.keyPressEvent
            self.addMenu(_new_menu)
            _actions = self._nodeDict[group_name]
            for x in _actions:
                _action = QtGui.QAction(x.display_name, self)
                _action.setText(x.display_name)
                _action.triggered.connect(self._on_search_submitted)
                self._actions[x.display_name] = _action
    ...
    def set_nodes(self, node_dict=None):
        if not self._nodeDict or self.rebuild:
            ...
            self._actions.clear()
            self._menus.clear()
            for group_name, node_types in node_dict.items():
                self._nodeDict[group_name] = node_types
