...
from ..core.define import EnumViewFeature
...
class NodeScene(QtWidgets.QGraphicsScene):
    ...
    def __init__(self, parent=None):
        ...
        self._gridMode = EnumViewFeature.GRID_DISPLAY_LINES.value
        self._gridColor = EnumViewFeature.GRID_COLOR.value
        self._bgColor = EnumViewFeature.BACKGROUND_COLOR.value
    ...
    def drawBackground(self, painter: QtGui.QPainter, rect: QtCore.QRectF) -> None:
        ...
        if self._gridMode is EnumViewFeature.GRID_DISPLAY_DOTS.value:
            _pen = QtGui.QPen(QtGui.QColor(*self.grid_color), 0.65)
            self._draw_dots(painter, rect, _pen, EnumViewFeature.GRID_SIZE.value)

        elif self._gridMode is EnumViewFeature.GRID_DISPLAY_LINES.value:
            _zoom = self.get_view().get_zoom()
            if _zoom > -0.5:
                _pen = QtGui.QPen(QtGui.QColor(*self.grid_color), 0.65)
                self._draw_grid(
                    painter, rect, _pen, EnumViewFeature.GRID_SIZE.value
                )
            ...
            self._draw_grid(
                painter, rect, _pen, EnumViewFeature.GRID_SIZE.value * 8
            )

        painter.restore()

    def mousePressEvent(self, event: QtWidgets.QGraphicsSceneMouseEvent):
        _selected_nodes = self.get_view().get_selected_nodes()
        if self.get_view():
    ...
    @grid_mode.setter
    def grid_mode(self, mode: EnumViewFeature = None):
        if mode is None:
