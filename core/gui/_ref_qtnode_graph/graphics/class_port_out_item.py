...
# ------------------------------------------------------------------------------
# Project       : 
# Sourcefile(s) : class_port_out_item.py
# ------------------------------------------------------------------------------
#
# File          : class_port_out_item.py
#
...
from ..core.define import EnumNodeStyleProperty
...
class PortOutputNodeItem(NodeItem):
    ...
    def _paint_horizontal(self, painter, option, widget):
        ...
        _transform.translate(_rect.x() + (_size / 3), _rect.center().y())
        _transform.rotate(-90)
        _poly = _transform.map(triangle)
        ...
        _edge_size = 30
        _edge_rect = QtCore.QRectF(_rect.x() + (_size * 1.6),
    ...
    def _paint_vertical(self, painter, option, widget):
        ...
        _text_rect = QtCore.QRectF(
            _rect.center().x() - (_text_rect.width() / 2) - 5,
            _rect.height() - _text_rect.height(),
            _text_rect.width() + 10,
            _text_rect.height()
        )
        ...
        _size = int(_rect.height() / 4)
        _triangle = QtGui.QPolygonF()
        _triangle.append(QtCore.QPointF(-_size, _size))
        _triangle.append(QtCore.QPointF(0.0, 0.0))
        _triangle.append(QtCore.QPointF(_size, _size))

        _transform = QtGui.QTransform()
        _transform.translate(_rect.center().x(), _rect.y() + (_size / 3))
        # transform.rotate(-90)
        _poly = _transform.map(_triangle)
    ...
    def _align_label_vertical(self, h_offset, v_offset):
        ...
        _y = _rect.height() - _text_rect.height() - 4.0
        self._textItem.setPos(_x + h_offset, _y + v_offset)
