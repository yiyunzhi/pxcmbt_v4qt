...
from ..core.define import Z_VAL_NODE_WIDGET, EnumPipeSlicerStyleProperty
...
class SlicerPipeItem(QtWidgets.QGraphicsPathItem):
    ...
    def paint(self, painter, option, widget=None):
        ...
        _text_color = QtGui.QColor(*EnumPipeSlicerStyleProperty.COLOR.value)
        _text_color.setAlpha(80)
        painter.setPen(QtGui.QPen(
            _text_color, EnumPipeSlicerStyleProperty.WIDTH.value, QtCore.Qt.PenStyle.SolidLine
        ))
        painter.drawText(_text_pos, _text)

        painter.setPen(QtGui.QPen(
            _color, EnumPipeSlicerStyleProperty.WIDTH.value, QtCore.Qt.PenStyle.DashDotLine
        ))
        painter.drawPath(self.path())

        _pen = QtGui.QPen(
            _color, EnumPipeSlicerStyleProperty.WIDTH.value, QtCore.Qt.PenStyle.SolidLine
        )
