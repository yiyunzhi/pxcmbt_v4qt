...
from .core.class_model import NodeGraphModel
...
from .core.exceptions import NodeCreationError, NodeDeletionError
from .widgets.class_view import NodeGraphView
from .widgets.class_node_graph_widget import NodeGraphWidget, SubGraphWidget
from .nodes.class_backdrop_node import BackdropNode
from .nodes.class_base_node import BaseNode
from .nodes.class_group_node import GroupNode
from .nodes.class_port_node import PortInputNode, PortOutputNode
...
class NodeGraph(QtCore.QObject):
    ...
    :emits: selected node
    """
    sigPortConnected = QtCore.Signal(Port, Port)
    """
    ...
    """
    sigPortDisconnected = QtCore.Signal(Port, Port)
    """
    ...
    def __init__(self, parent: QtCore.QObject = None, **kwargs):
        ...
        Args:
            ...
            **kwargs : Used for overriding internal objects at init time.
        """
        super(NodeGraph, self).__init__(parent)
        self.setObjectName('NodeGraph')
        self._model = kwargs.get('model') or NodeGraphModel()
        _layout_direction = kwargs.get('layout_direction')
        if _layout_direction:
            if _layout_direction not in [e.value for e in EnumLayoutDirection]:
                _layout_direction = EnumLayoutDirection.HORIZONTAL.value
            self._model.layoutDirection = _layout_direction
        else:
            _layout_direction = self._model.layoutDirection
        self._nodeFactory = kwargs.get('node_factory') or NodeFactory()
        self._undoView = None
        self._undoStack = kwargs.get('undo_stack') or QtGui.QUndoStack(self)
        self._widget = None
        self._subGraphs = {}
        self._view = kwargs.get('view') or NodeGraphView(undo_stack=self._undoStack)
        self._view.set_layout_direction(_layout_direction)
        self._contextMenu = {}
    ...
    def _register_context_menu(self):
        ...
        if _menus.get('graph'):
            self._contextMenu['graph'] = NodeGraphMenu(self, _menus['graph'])
        if _menus.get('nodes'):
            self._contextMenu['nodes'] = NodesMenu(self, _menus['nodes'])
    ...
    def _wire_signals(self):
        ...
        # internal signals.
        self._view.sigSearchTriggered.connect(self._on_search_triggered)
        self._view.sigConnectionSliced.connect(self._on_connection_sliced)
        self._view.sigConnectionChanged.connect(self._on_connection_changed)
        self._view.sigMovedNodes.connect(self._on_nodes_moved)
        self._view.sigNodeDoubleClicked.connect(self._on_node_double_clicked)
        self._view.sigNodeNameChanged.connect(self._on_node_name_changed)
        self._view.sigBackdropNodeUpdated.connect(self._on_node_backdrop_updated)
        self._view.sigNodeInserted.connect(self._on_insert_node)

        # pass through translated signals.
        self._view.sigNodeSelected.connect(self._on_node_selected)
        self._view.sigNodeSelectionChanged.connect(self._on_node_selection_changed)
        self._view.sigDataDropped.connect(self._on_node_data_dropped)
    ...
    def _on_insert_node(self, pipe, node_id, prev_node_pos):
        ...
        if not isinstance(_node, BaseNode):
            return

        _disconnected = [(pipe.input_port, pipe.output_port)]
        _connected = []

        if _node.get_input_ports():
            _connected.append(
                (pipe.output_port, _node.get_input_ports()[0].view)
            )
        if _node.get_output_ports():
            _connected.append(
                (_node.get_output_ports()[0].view, pipe.input_port)
            )

        self._undoStack.beginMacro('inserted node')
        self._on_connection_changed(_disconnected, _connected)
    ...
    def _on_nodes_moved(self, node_data):
        ...
        for node_view, prev_pos in node_data.items():
            _node = self._model.nodes[node_view.id]
            self._undoStack.push(NodeMovedCmd(_node, _node.get_pos(), prev_pos))
        self._undoStack.endMacro()
    ...
    def toggle_node_search(self):
        ...
        if self._view.underMouse() or self._view.contentsRect().contains(_cursor_pos):
            self._view.tab_search_set_nodes(self._nodeFactory.names)
    ...
    def set_grid_mode(self, mode=None):
        ...
        """
        _display_feature = [
            EnumViewFeature.GRID_DISPLAY_NONE.value,
            EnumViewFeature.GRID_DISPLAY_DOTS.value,
            EnumViewFeature.GRID_DISPLAY_LINES.value
        ]
        if mode not in _display_feature:
            mode = EnumViewFeature.GRID_DISPLAY_LINES.value
        self.get_scene().grid_mode = mode
        self._view.force_update()
    ...
    def set_acyclic(self, mode=False):
        ...
        self._view.acyclic = mode

    def get_pipe_collision(self):
        """
        ...
        """
        return self._model.pipeCollision
    ...
    def set_pipe_collision(self, mode=True):
        ...
        self._view.pipe_collision = mode

    def set_pipe_style(self, style=EnumPipeShape.CURVED.value):
        """
        ...
        """
        _pipe_max = max([EnumPipeShape.CURVED.value,
                         EnumPipeShape.STRAIGHT.value,
                         EnumPipeShape.ANGLE.value])
        _style = style if 0 <= style <= _pipe_max else EnumPipeShape.CURVED.value
        self._view.set_pipe_layout(style)
    ...
    def set_layout_direction(self, direction):
        ...
        self._model.layoutDirection = direction
    ...
    def create_node(self, node_type, name=None, selected=True, color=None,
        ...
        if _node:
            ...
            if color:
                _node.model.color = format_color(color)
            if text_color:
                _node.model.text_color = format_color(text_color)
            if pos:
                _node.model.pos = [float(pos[0]), float(pos[1])]

            # initial node direction layout.
            _node.model.layoutDirection = self.get_layout_direction()

            _node.update()

            _undo_cmd = NodeAddedCmd(self, _node, _node.model.pos)
            if push_undo:
                _undo_label = 'create node: "{}"'.format(_node.NODE_NAME)
                ...
                NodeAddedCmd(self, _node, _node.model.pos).redo()
    ...
    def add_node(self, node, pos=None, selected=True, push_undo=True):
        ...
        _wid_types = node.model.__dict__.pop('_TEMP_property_widget_types')
        _prop_attrs = node.model.__dict__.pop('_TEMP_property_attrs')
        ...
        if self.model.get_node_common_properties(node.type_) is None:
            ...
            for pname, pattrs in _prop_attrs.items():
                _node_attrs[node.type_][pname].update(pattrs)
            self.model.set_node_common_properties(_node_attrs)
        ...
        # initial node direction layout.
        node.model.layout_direction = self.get_layout_direction()

        # update method must be called before it's been added to the viewer.
        node.update()

        if push_undo:
            self._undoStack.beginMacro('add node: "{}"'.format(node.get_name()))
            self._undoStack.push(NodeAddedCmd(self, node, pos))
            if selected:
    ...
    def delete_node(self, node, push_undo=True):
        ...
        _node_id = node.id
        if push_undo:
            self._undoStack.beginMacro('delete node: "{}"'.format(node.get_name()))
    ...
    def get_selected_nodes(self):
        ...
        """
        _nodes = []
        for item in self._view.get_selected_nodes():
            _node = self._model.nodes[item.id]
            _nodes.append(_node)
        return _nodes
            ...
            if isinstance(n, BaseNode):
                ...
                for p in n.get_output_ports():
                    if p.is_locked():
                        p.set_locked(False, connected_ports=False)
                    p.clear_connections()
            self._undoStack.push(NodeRemovedCmd(self, n))
        self._undoStack.clear()
        self._model.session = ''
    ...
    def _serialize(self, nodes):
        ...
        _serial_data['graph']['pipe_collision'] = self.get_pipe_collision()
    ...
    def _on_close_sub_graph_request(self, index: int):
        ...
        Args:
            index (int): tab index.
        """
        _node_id = self.widget.tabToolTip(index)
    ...
    def expand_group_node(self, node):
        ...
        self.widget.add_view(_sub_graph.widget, node.get_name(), node.id)
...
class SubGraph(NodeGraph):
    ...
    def __init__(self, parent: NodeGraph = None, node=None, node_factory: NodeFactory = None, **kwargs):
        """
        Args:
            parent (object): object parent.
        ...
        """
        NodeGraph.__init__(self, parent, node_factory=node_factory, **kwargs)

        # sub graph attributes.
        self._node = node
        self._parentGraph = parent
        self._subViewWidget = None
        if self._parentGraph.is_root:
            self._initializedGraphs = [self]
            self._subGraphs[self._node.id] = self
        else:
    ...
    def __repr__(self):
        return '<{}("{}") object at {}>'.format(
            self.__class__.__name__, self._node.get_name(), hex(id(self)))
