...
from ..core.define import EnumNodePropWidgetType
...
class BackdropNode(NodeObject):
    ...
    **Inherited from:** :class:`NodeGraphQt.NodeObject`
    ...
    def __init__(self, graphics_item=None):
        super(BackdropNode, self).__init__(graphics_item or BackdropNodeItem)
        # override base default color.
        self.model.color = (5, 129, 138, 255)
        self.create_property('backdropText', '',
                             widget_type=EnumNodePropWidgetType.QTEXT_EDIT.value,
                             tab='Backdrop')
    ...
    def on_backdrop_updated(self, update_prop, value=None):
        ...
        if update_prop == 'sizer_mouse_release':
            self.graph.begin_undo('resized "{}"'.format(self.get_name()))
            ...
            self.graph.begin_undo('"{}" auto resize'.format(self.get_name()))
            self.set_property('width', value['width'])
            self.set_property('height', value['height'])
            self.set_pos(*value['pos'])
            self.graph.end_undo()
    ...
    def auto_size(self):
        ...
        self.set_property('width', _size['width'])
        self.set_property('height', _size['height'])
        self.set_pos(*_size['pos'])
        self.graph.end_undo()
    ...
    def wrap_nodes(self, nodes):
        ...
        if not nodes:
            return
        self.graph.begin_undo('"{}" wrap nodes'.format(self.get_name()))
    ...
    def set_text(self, text=''):
        ...
        """
        self.set_property('backdropText', text)
    ...
    def get_text(self):
        ...
        Returns:
            str: text string.
        """
        return self.get_property('backdropText')
