...
class NodeGraphMenu(object):
    ...
    def __init__(self, graph, q_menu):
        self._graph = graph
        self._qMenu = q_menu
        self._name = q_menu.title()
        self._menus = {}
        self._commands = {}
        self._items = []
    ...
    def add_menu(self, name):
        ...
        _base_menu = BaseMenu(name, self._qMenu)
        self._qMenu.addMenu(_base_menu)
...
class NodesMenu(NodeGraphMenu):
    ...
    **Inherited from:** :class:`NodeGraphQt.NodeGraphMenu`
    ...
    def add_command(self, name, func=None, node_type=None, node_class=None):
        ...
        _node_menu = self.qmenu.get_menu(node_type)
        if not _node_menu:
            _node_menu = BaseMenu(node_type, self.qmenu)

            if node_class:
                _node_menu.node_class = node_class
                _node_menu.graph = self._graph

            self.qmenu.addMenu(_node_menu)
        ...
        if LooseVersion(QtCore.qVersion()) >= LooseVersion('5.10'):
            _action.setShortcutVisibleInContextMenu(True)
        if func:
            _action.executed.connect(func)

        if node_class:
            _node_menus = self.qmenu.get_menus(node_class)
