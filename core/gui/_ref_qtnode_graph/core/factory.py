...
class NodeFactory(object):
    ...
    def register_node(self, node, alias=None):
        ...
        _node_type = node.type_

        if self.__nodes.get(_node_type):
            raise NodeRegistrationError(
                'node type "{}" already registered to "{}"! '
                'Please specify a new plugin class name or __identifier__.'
                .format(_node_type, self.__nodes[_node_type]))
        self.__nodes[_node_type] = node

        if self.__names.get(_name):
            self.__names[_name].append(_node_type)
        else:
            self.__names[_name] = [_node_type]
        ...
        if alias:
            ...
            self.__aliases[alias] = _node_type
