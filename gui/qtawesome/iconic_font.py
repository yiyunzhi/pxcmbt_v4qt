...
def text_color():
    try:
        ...
        return palette.color(QtGui.QPalette.Active, QtGui.QPalette.Text)
...
def text_color_disabled():
    try:
        palette = QtWidgets.QApplication.instance().palette()
        return palette.color(QtGui.QPalette.Disabled, QtGui.QPalette.Text)
    except AttributeError:
    ...
    def _paint_icon(self, iconic, painter, rect, mode, state, options):
        ...
        def try_draw_rawfont():
            if draw == 'glyphrun' and animation is not None:
                ...
                # See spyder-ide/qtawesome#39
                rawfont = iconic.rawfont(prefix, draw_size, QtGui.QFont.PreferNoHinting)
            else:
            ...
            size = QtCore.QSizeF(abs(advance.x()), ascent + rawfont.descent())
            painter.translate(QtCore.QRectF(rect).center())
            ...
            if draw == 'path':
                ...
                if QtGui.QGlyphRun:
                    ...
                    glyphrun.setPositions((QtCore.QPointF(0, ascent),))
                    painter.drawGlyphRun(QtCore.QPointF(0, 0), glyphrun)
                else:
                    warnings.warn("QGlyphRun is unavailable for the current Qt binding! "
                                  "QtAwesome will use the default draw values")
                    return False
            elif draw == 'image':
                image = rawfont.alphaMapForGlyph(glyph, QtGui.QRawFont.PixelAntialiasing) \
                    .convertToFormat(QtGui.QImage.Format_ARGB32_Premultiplied)
        ...
        if draw == 'text' or not try_draw_rawfont():
            ...
            painter.drawText(rect, int(QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter), char)
...
class IconicFont(QtCore.QObject):
    ...
    def load_font(self, prefix, ttf_filename, charmap_filename, directory=None):
        ...
        if QtWidgets.QApplication.instance() is not None:
            with open(os.path.join(directory, ttf_filename), 'rb') as font_data:
                data = font_data.read()
                id_ = QtGui.QFontDatabase.addApplicationFontFromData(data)
            font_data.close()
    ...
    def rawfont(self, prefix, size, hintingPreference=QtGui.QFont.PreferDefaultHinting):
        ...
        tid = str(QtCore.QThread.currentThread())
        ...
        if tid not in cache:
            ...
            QtCore.QThread().currentThread().finished.connect(clear_cache)
        key = prefix, size, hintingPreference
        if key not in cache[tid]:
            cache[tid][key] = QtGui.QRawFont(self.fontdata[prefix], size, hintingPreference)
